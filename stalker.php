...
$mac='00:1A:78:43:44:51';


$sn = strtoupper (substr(md5($mac),13)); //hashlib.md5(mac).hexdigest().upper()[13:];
$device_id = strtoupper (hash('sha256', $sn)); //hashlib.sha256(sn).hexdigest().upper();
$device_id2 = strtoupper (hash('sha256', $mac)); //hashlib.sha256(mac).hexdigest().upper();
$signature = strtoupper (hash('sha256', $sn . $mac)); //hashlib.sha256(sn + mac).hexdigest().upper();
...
$headers[] = 'Referer: http://portal.iptvprivateserver.tv/stalker_portal/c/'; //Your referrer address
...
curl_setopt($ch, CURLOPT_POSTFIELDS,'action=handshake&type=stb&JsHt tpRequest=1-xml');
...
$headers1[] = 'Referer: http://portal.iptvprivateserver.tv/stalker_portal/c/'; //Your referrer address
...
curl_setopt($ch1, CURLOPT_POSTFIELDS,'stb_type=MAG254&num_banks=1&Js HttpRequest=1-xml&hw_version=2.6-IB-00&hd=1&not_valid_token=0&image_version=218&ver=Im ageDescription%3A%25200.2.18-r11-pub-254%3B%2520ImageDate%3A%2520Wed%2520Mar%252018%252 018%3A09%3A40%2520EET%25202015%3B%2520PORTAL%2520v ersion%3A%25204.9.14%3B%2520API%2520Version%3A%252 0JS%2520API%2520version%3A%2520331%3B%2520STB%2520 API%2520version%3A%2520141%3B%2520Player%2520Engin e%2520version%3A%25200x572&auth_second_step=0&acti on=get_profile&type=stb&' . $cadena);
...
$headers2[] = 'Referer: http://portal.iptvprivateserver.tv/stalker_portal/c/'; //Your referrer address
$headers2[] = 'Content-Type: application/x-www-form-urlencoded';
//$headers2[] = 'Content-Length: 129';
$headers2[] = 'Accept: */*';
$headers2[] = 'Expect: 100-continue';
$headers2[] = 'Connection: Keep-Alive';
$headers2[] = 'X-User-Agent: Model: MAG254; Link: Ethernet';
$headers2[] = 'Authorization: Bearer ' . $keyy; //2E50B32222DD7F8CF8F31FEE7F189EFB
...
curl_setopt($ch2, CURLOPT_POSTFIELDS,'cmd=ffrt2+http%3A%2F%2Flocalho st%2Fch%2F' . $cm . '&JsHttpRequest=1-xml&disable_ad=0&forced_storage=undefined&action=c reate_link&type=itv');
...
$pos = strpos($token2, 'token');
$token=substr($token2,$pos+6,39-7);
...
header('Location: http://31.220.40.118:80/LiveEdge/' . $canal . '/playlist.m3u8?token=' . $token);
